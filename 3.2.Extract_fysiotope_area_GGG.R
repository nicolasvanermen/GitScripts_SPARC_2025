...
fysiotopes_agg <- as.data.frame(
  cbind(NULL, Fysiotoop = c("buiten estuarien gebied","supralitoraal", "hoog slik", "middelhoog slik", "laag slik", 
...
fysiotopes_PL <- fysiotopes_agg %>% 
  ...
  mutate(
    ...
    Area = recode(Area, "Wal Zwijn" = "Wal & Zwijn")) %>%
...
# nog iets bedenken voor GGG's...
ggplot(fysiotopes_PL  %>% filter(Area %in% areas_GGG$area), aes(x = Area, y = Ecotope_ha, fill = Fysiotope)) +
  geom_bar(stat = "identity") + 
  labs(x = "", y = "Surface area (ha)", fill = "FCA-CRT physiotopes") +
...
ggsave(paste0("./Exports/SPARC_ecotope_CRT_areas_ENG.jpeg"),
       dpi = 300,
       width = 7, height = 5)
